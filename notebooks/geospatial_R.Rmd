...
```{r}
install.packages("sf") #simple features library
install.packages("leaflet") #good for visualizations
library(sf)
library(tidyverse)
```
...
```{r}
zipcodes <- st_read('../data/zipcodes.geojson') # reads in geographical info
```
...
Let's extract the latitude and longitude using the str_extract function from stringr. (Look at stringr cheat sheat!!)
```{r}
bus_stops <- bus_stops |> 
  mutate(
...
bus_stops_geo <- st_as_sf(
  ...
  crs = st_crs(zipcodes) #coordinate reference system
)
```
...
```{r}
bus_zips <- st_join(bus_stops_geo, zipcodes, join = st_within, left = FALSE)
bus_zips
```
```{r}
zipcodes |> 
ggplot() +
geom_sf() +
geom_sf(data = bus_zips, size=0.1)
```
